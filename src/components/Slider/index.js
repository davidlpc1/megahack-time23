...
export default function Slider({ className }) {
  const [sliderClassName, setSliderClassName] = useState(
    "flex justify-center align-center h-64 w-full bg-pink-100 text-white mx-2 text-xl"
  );
  ...
  function openDiv({ target }) {
    ...
    setSliderClassName(
      "flex justify-center align-center h-64 w-full bg-pink-100 text-white mx-2 text-xl hidden"
    );

    const isImagetoUpper =
      target.alt !== "Close" && target.childElementCount !== 2;
    const isClose = target.alt === "Close";
  ...
  return (
    <div className={className}>
      <h1 className="text-center p-8 text-4xl text-logo">
        Entenda os termos da economia
      </h1>
      <div className="App outline-none">
        <Carousel
          className="w-full"
          breakPoints={itemsToShow}
          enableAutoPlay={sliderClassName.indexOf("hidden", 0) === -1}
          autoPlaySpeed={7000}
        >
          {imgs.map((img, index) => {
            return (
              ...
              >
                ...
                />
                <img
                  className="h-2 sm:h-4 md:h-4 lg:h-8 xl:h-10 hidden m-4 cursor-pointer"
                  src={close}
                  alt="Close"
                  onClick={closeDiv}
                />
